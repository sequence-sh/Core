...
public abstract partial class StepTestBase<TStep, TOutput>
{
    ...
    [Theory]
    [NonStaticMemberData(nameof(StepCaseNames), true)]
    public async Task Should_behave_as_expected_when_serialized_deserialized_and_run(
        string stepCaseName)
    {
        ...
        #pragma warning restore CA1034 // Nested types should not be visible
    {
        ...
        protected StepCase(
            string name,
            IStep step,
            Maybe<TOutput> expectedOutput,
            string[] expectedLoggedValues) : base(
            expectedLoggedValues.Select(CompressNewlines).ToList()
        )
        {
            Name           = name;
            Step           = step;
