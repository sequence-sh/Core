...
public sealed class MultipleTypeReference : ITypeReference, IEquatable<ITypeReference>
{
    ...
    /// </summary>
    public static Result<ITypeReference, IErrorBuilder> TryCreate(
        ...
        string parentStep)
    {
        ...
        switch (set.Count)
        {
            case 0: return new ActualTypeReference(typeof(object)); //TODO type reference any???
            case 1: return Result.Success<ITypeReference, IErrorBuilder>(set.Single());
            default:
            {
    ...
    public bool Equals(ITypeReference? other)
    {
        ...
        return other switch
        {
            ActualTypeReference actualType => AllReferences.Count == 1
                                           && AllReferences.Contains(actualType),
            MultipleTypeReference multipleTypeReference => AllReferences.SetEquals(
                multipleTypeReference
                    .AllReferences
            ),
            VariableTypeReference variableTypeReference => AllReferences.Count == 1
                                                        && AllReferences.Contains(
                                                               variableTypeReference
                                                           ),
            GenericTypeReference genericTypeReference => AllReferences.Count == 1
                                                      && AllReferences.Contains(
    ...
    public override int GetHashCode() =>
        AllReferences.Select(x => x.GetHashCode()).FirstOrDefault()!;

    /// <inheritdoc />
    public IEnumerable<VariableTypeReference> VariableTypeReferences =>
        AllReferences.SelectMany(x => x.VariableTypeReferences);
        ...
        TypeResolver typeResolver)
    {
        var results = AllReferences
            ...
            .Bind(
                x => x.Distinct()
                    .EnsureSingle(new ErrorBuilder(ErrorCode.CannotInferType) as IErrorBuilder)
            );
