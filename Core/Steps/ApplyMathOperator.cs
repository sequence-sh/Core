...
public sealed class ApplyMathOperator : CompoundStep<int>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (@operator.IsFailure)
            return @operator.ConvertFailure<int>();
        ...
        var result = @operator.Value switch
        {
            ...
            MathOperator.Divide when right.Value == 0 => Result.Failure<int, IError>(
                new SingleError(new StepErrorLocation(this), ErrorCode_Core.DivideByZero)
            ),
...
public class ApplyMathOperatorStepFactory : SimpleStepFactory<ApplyMathOperator, int>
{
    ...
    /// The instance.
    /// </summary>
    public static SimpleStepFactory<ApplyMathOperator, int> Instance { get; } =
        new ApplyMathOperatorStepFactory();
